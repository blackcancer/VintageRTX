...
public sealed class EmissionAssetTests
{
    ...
    public static void Initialize(TestContext context)
    {
        ...
        Context.Resolving += ResolveGameAssembly;
    }
    [ClassCleanup]
    public static void Cleanup() => Context.Resolving -= ResolveGameAssembly;
    ...
    private static ModSystem NativePatcher()
    {
        ...
        Type type = assembly.GetType("Vintagestory.ServerMods.NoObf.ModJsonPatchLoader",throwOnError:true)!;
        return (ModSystem)Activator.CreateInstance(type)!;
    }

    [TestMethod]
    public void LoadedJsonLibraryIsExactlyTheOfficialClientDependency()
    {
        if (GameTestIsolation.InvokeIfDefault(typeof(EmissionAssetTests),nameof(LoadedJsonLibraryIsExactlyTheOfficialClientDependency))) return;
        Assembly json = typeof(JToken).Assembly;
        Assert.AreSame(Context, AssemblyLoadContext.GetLoadContext(json));
        string expected = Path.Combine(game,"Lib","Newtonsoft.Json.dll");
        CollectionAssert.AreEqual(SHA256.HashData(File.ReadAllBytes(expected)),SHA256.HashData(File.ReadAllBytes(json.Location)));
        Assert.IsNotNull(typeof(JToken).GetMethod("WriteTo",new[]{typeof(JsonWriter)}));
        Console.WriteLine("Official JSON dependency: {0}; sha256={1}",json.FullName,Convert.ToHexString(SHA256.HashData(File.ReadAllBytes(expected))));
    }

    [TestMethod]
    public void ActualNativePatcherAppliesShippedExampleBeforeCatalogConsumption()
    {
        if (GameTestIsolation.InvokeIfDefault(typeof(EmissionAssetTests),nameof(ActualNativePatcherAppliesShippedExampleBeforeCatalogConsumption))) return;
        var world = new MemoryContext(Asset());
        world.Add("vrtxemissionexample:patches/emission.json",File.ReadAllText(Path.Combine(AppContext.BaseDirectory,"Fixtures","example-patch.json")));
        ...
        Assert.AreEqual(10,settings.Catalog.ProfileCount);Assert.IsNotNull(settings.Fingerprint);
    }

    [TestMethod]
    public void ActualAttributePatchPreservesUnrelatedFieldsAndOverridesIntrinsicDefault()
    {
        if (GameTestIsolation.InvokeIfDefault(typeof(EmissionAssetTests),nameof(ActualAttributePatchPreservesUnrelatedFieldsAndOverridesIntrinsicDefault))) return;
        var world = new MemoryContext(Asset());
        ...
        var settings = new EmissionAssetCatalog(world.Assets,world.Logger);Assert.IsTrue(settings.LoadPatched());
        EmissionSelection result = settings.Resolve("example:torchbot",EmissionTarget.Entity,new JsonObject(document["attributes"]!));
    ...
    {
        if (GameTestIsolation.InvokeIfDefault(typeof(EmissionAssetTests),nameof(ModSystemLoadsOnlyAfterAssetsStageAndRespectsClientSide))) return;
        var world = new MemoryContext(Asset());var system = new VintageRTXModSystem();
        Assert.IsTrue(system.ShouldLoad(EnumAppSide.Client));Assert.IsFalse(system.ShouldLoad(EnumAppSide.Server));
        system.Start(world.Api);Assert.AreEqual(0,world.Reads);
        system.AssetsLoaded(world.Api);Assert.AreEqual(1,world.Reads);
    ...
    {
        if (GameTestIsolation.InvokeIfDefault(typeof(EmissionAssetTests),nameof(InvalidReloadPreservesCatalogAndAlreadyResolvedTypes))) return;
        ...
        world.Catalog.Text = "{broken";Assert.IsFalse(settings.LoadPatched());
        Assert.AreEqual(revision,settings.Revision);Assert.AreEqual(hash,settings.Fingerprint);
        Assert.AreSame(before,settings.Resolve("game:candle",EmissionTarget.Item,null));Assert.IsNotNull(settings.LastError);
    }

    [TestMethod]
    public void SettingsAreNotParsedAgainForEverySourceOrFrame()
    {
        if (GameTestIsolation.InvokeIfDefault(typeof(EmissionAssetTests),nameof(SettingsAreNotParsedAgainForEverySourceOrFrame))) return;
        var world = new MemoryContext(Asset());var settings = new EmissionAssetCatalog(world.Assets,world.Logger);Assert.IsTrue(settings.LoadPatched());
    ...
    {
        if (GameTestIsolation.InvokeIfDefault(typeof(EmissionAssetTests),nameof(BadAttributeIsRejectedOnceWithoutAFlameFallback))) return;
        var world = new MemoryContext(Asset());var settings = new EmissionAssetCatalog(world.Assets,world.Logger);Assert.IsTrue(settings.LoadPatched());
        var attributes = new JsonObject(JObject.Parse("""{"vintageRtxEmission":{"profiel":"vintagertx:candle"}}"""));
        for(int i=0;i<100;i++)Assert.IsNull(settings.Resolve("game:candle",EmissionTarget.Block,attributes).CreateLight(default,Vector3.One));
    ...
    {
        if (GameTestIsolation.InvokeIfDefault(typeof(EmissionAssetTests),nameof(SuccessfulReloadInvalidatesTypeCacheButUnchangedBytesDoNot))) return;
        var world = new MemoryContext(Asset());var settings = new EmissionAssetCatalog(world.Assets,world.Logger);Assert.IsTrue(settings.LoadPatched());
        EmissionSelection before = settings.Resolve("game:candle",EmissionTarget.Block,null);
        Assert.IsTrue(settings.LoadPatched());Assert.AreSame(before,settings.Resolve("game:candle",EmissionTarget.Block,null));
        JObject json = JObject.Parse(world.Catalog.Text);json["profiles"]!["vintagertx:candle"]!["amplitude"] = .01;world.Catalog.Text = json.ToString();
        Assert.IsTrue(settings.LoadPatched());Assert.AreEqual(2L,settings.Revision);
        Assert.AreEqual(.01,settings.Resolve("game:candle",EmissionTarget.Block,null).Profile.Amplitude);Assert.AreEqual(.12,before.Profile.Amplitude);
    }

    [TestMethod]
    public void OfficialCandleAndChandelierDefinitionsHaveMappedRuntimeFamilies()
    {
        if (GameTestIsolation.InvokeIfDefault(typeof(EmissionAssetTests),nameof(OfficialCandleAndChandelierDefinitionsHaveMappedRuntimeFamilies))) return;
        EmissionCatalog catalog = EmissionCatalog.Parse(Asset());bool candle=false,chandelier=false;
        foreach(string file in Directory.EnumerateFiles(Path.Combine(game,"assets"),"*.json",SearchOption.AllDirectories))
        {
            ...
            if(!filename.Contains("candle",StringComparison.OrdinalIgnoreCase) && !filename.Contains("chandelier",StringComparison.OrdinalIgnoreCase))continue;
            JObject json=JObject.Parse(File.ReadAllText(file));string? code=json["code"]?.Value<string>();if(code is null)continue;
            if(code is "candle" or "candles" or "bunchocandles")candle=true;else if(code=="chandelier")chandelier=true;else continue;
            EmissionSelection selection=catalog.Resolve("game:"+code,normalized.Contains("/blocktypes/")?EmissionTarget.Block:EmissionTarget.Item);
        ...
        Assert.IsTrue(candle,"No official candle asset was examined.");Assert.IsTrue(chandelier,"No official chandelier asset was examined.");
    }
    ...
    private sealed class MemoryContext
    {
        ...
        internal MemoryContext(string json)
        {
            ...
            Assets=Stub.Create<IAssetManager>((method,args)=>method.Name switch
            {
                ...
                "GetMany"=>entries.Values.Where(value=>value.Location.Path.StartsWith((string)args[0]!,StringComparison.Ordinal)).Select(value=>value.Asset).ToList(),
            ...
            {
                "get_Assets"=>Assets,"get_Logger"=>Logger,"get_World"=>world,"get_Side"=>EnumAppSide.Client,
        ...
        internal MemoryAsset(string location,string text)
        {
            ...
            Asset=Stub.Create<IAsset>((method,args)=>
            {
                switch(method.Name)
                {
                    ...
                    case "ToObject":return JsonConvert.DeserializeObject(Text,method.GetGenericArguments()[0],args.FirstOrDefault() as JsonSerializerSettings);
...
/// <summary>Unexpected world reads fail the case; this is not a substitute JSON patcher.</summary>
